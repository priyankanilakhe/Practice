...
```   Arrays.sort(arr, Collections.reverseOrder());
   ...
   2s array : Arrays.deepToString(newMatrix);
   1d array :Arrays.String(newMatrix);
   remove duplicates : Arrays.stream(arr).distinct().toArray();
   Arrays.sort(B, (a, b) -> Integer.compare(a%2, b%2));
   Arrays.sort(intervals ,(a,b)-> Integer.compare(a[0], b[0]));
   Arrays.sort(boxes,(a,b)->Integer.compare(b[1],a[1]));
	Arrays.sort(boxes,(a,b)->b[1]-a[1]);
   Binary Conversion : n%2 + 10*decimalToBinary(n/2);
   Collection.sort() =nlogn time complexity
    Array.sort() =nlogn time complexity
   System.arraycopy(arr, 0, newarr, 0, arr.length);
		 System.arraycopy(arr, 0, newarr, arr.length, arr.length);
 
   
   String.valueOf(suffle); : valueOf() method converts different types of values into string. By the help of string valueOf() method, you can convert int to string, long to string, boolean to string, character to string, float to string, double to string, object to string and char array to string.
   StringBuilder s= new StringBuilder(value); s.setCharAt(indices[i], s.charAt(i));
   String s={} s.repeat(); //Java 11 added 
   
   PriorityQueue<Entry<Integer,Integer>> maxHeap = new PriorityQueue<>((a,b)-> map.get(b)-map.get(a)); // sTORE THE HEAP MAX SORT 
```
